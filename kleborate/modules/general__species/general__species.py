...
def test_add_cli_options():
    ...
    assert '--general__species_strong' in parser.format_help()
    assert '--general__species_weak' in parser.format_help()


def add_cli_options(parser):
    module_name = os.path.basename(__file__)[:-3]
    group = parser.add_argument_group(f'{module_name} module')
    group.add_argument('--general__species_strong', type=float, default=0.02,
                       help='Mash distance threshold for a strong species match')
    group.add_argument('--general__species_weak', type=float, default=0.04,
                       help='Mash distance threshold for a weak species match')
    return group


def check_cli_options(args):
    if args.general__species_strong <= 0.0 or args.general__species_strong >= 1.0:
        sys.exit('Error: --general__species_strong must be between 0.0 and 1.0')
    if args.general__species_weak <= 0.0 or args.general__species_weak >= 1.0:
        sys.exit('Error: --general__species_weak must be between 0.0 and 1.0')
    if args.general__species_weak < args.general__species_strong:
        sys.exit('Error: --general__species_weak must be greater than '
                 '--general__species_strong')
...
def get_results(assembly, minimap2_index, args, previous_results):
    ...
    if distance <= args.general__species_strong:
        species_hit_strength = 'strong'
    elif distance <= args.general__species_weak:
