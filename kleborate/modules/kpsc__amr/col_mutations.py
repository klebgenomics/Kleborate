...
def check_for_mgrb_pmrb_gene_truncations(hits_dict, assembly, trunc, min_ident):
    ...
    if mgrb_frameshift:
        ...
        truncations.append(("mgrB:del", {"Genetic_variation_type": "Gene deletion", "Coverage": "0.00%"}))
    else:
        if mgrb_query_seq:
            mgrb_start_codon = mgrb_query_seq[:3].upper()
            if mgrb_start_codon not in start_codons:
                truncations.append(('mgrB:p.(Met1?)', {**mgrb_hit_data, 'Genetic_variation_type': 'Inactivating mutation detected'}))
    ...
    if pmrb_frameshift:
        ...
        truncations.append(("pmrB:del", {"Genetic_variation_type": "Gene deletion", "Coverage": "0.00%"}))
    else:
        if pmrb_query_seq:
            pmrb_start_codon = pmrb_query_seq[:3].upper()
            if pmrb_start_codon not in start_codons:
                truncations.append(('pmrB:p.(Met1?)', {**pmrb_hit_data, 'Genetic_variation_type': 'Inactivating mutation detected'}))
